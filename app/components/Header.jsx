  ...
  return (
    <header>
      ...
      <div className='hidden w-full  z-[999] h-fit md:flex transition-all duration-300 fixed top-0'>
        <div className='bg-white container flex flex-row justify-between items-center h-fit mx-auto py-2 px-10'>
          <Link 
            href="#"
            className=' w-[170px] transition-all duration-300 h-[80px] p-[.6rem] rounded-full'
          >
              <Image src={'/logo.svg'} alt='logo' width={170} height={22} className='w-full h-full object-contain'/>
          </Link>
          <div className='flex transition-all duration-300 w-fit h-[90px] justify-end'>
            <Nav containerStyles="flex justify-between items-center " linkStyles="w-fit flex justify-center items-center !no-underline"/>
          </div>
        </div>
      </div>
      ...
      <div className='md:hidden w-full fixed left-0 right-0 bottom-0 top-0 h-full z-[999] pointer-events-none flex flex-col items-end'>
        <div className={`p-8 flex w-[70%] ${isActive && "bg-black"} justify-end items-center`}>
          <button 
            className='h-[4rem] w-[4rem] pointer-events-auto rounded-full transition-all duration-300 bg-[#ff0187]'
